...
@Component({
  ...
  providers: [NguiAutoComplete, SearchItemService],
  styles: [`
  @keyframes slideDown {
    0% {
...
export class NguiAutoCompleteComponent implements OnInit {
  ...
  constructor(
    ...
    public autoComplete: NguiAutoComplete, private router: Router, private searchItemService: SearchItemService
  ) {
  ...
  selectOne(data: any) {
    ...
    this.router.navigate(['home/category/'+ data.id]);
  };
    ...
    switch (evt.keyCode) {
      ...
      case 13: // ENTER, choose it!!
        if (this.filteredList.length > 0) {
          ...
          this.searchItemService.getSku(evt.target.value).subscribe(page => {
              
              if (page.content.length != 0 ){
               var itemName =  page.content[0].item.name.toLowerCase().replace(/ /g, '-');
