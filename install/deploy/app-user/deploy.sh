...
if echo $@ | grep '\-\-build' &>/dev/null; then 
  ...
  docker-compose --project-name eta --file $deployPath/docker-compose.yml build appuser
...
if docker-compose --project-name eta --file $deployPath/docker-compose.yml up -d appuser; then
