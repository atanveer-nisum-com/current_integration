...
if echo $@ | grep '\-\-build' &>/dev/null; then 
  ...
  docker-compose --project-name eta --file $deployPath/docker-compose.yml build apppromotion
...
docker stop apppromotion
docker rm apppromotion

####################################
# STARTING CONTAINER
####################################

if docker-compose --project-name eta --file $deployPath/docker-compose.yml up -d apppromotion; then
