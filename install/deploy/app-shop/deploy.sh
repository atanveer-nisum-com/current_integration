...
if echo $@ | grep '\-\-build' &>/dev/null; then 
  ...
  docker-compose --project-name eta --file $deployPath/docker-compose.yml build appshop
...
if docker-compose --project-name eta --file $deployPath/docker-compose.yml up -d appshop; then
